...
describe('Token Service', () => {
  ...
  describe('Given there is an error calling KMS', () => {
    it('Throws an error', async () => {
      ...
      await (expect(tokenService.mintToken(payload))
        .rejects.toThrow('Mock failing to sign JWT error message'))
    })
  })
  ...
  describe('Given the request to KMS is successful', () => {
    describe('Given KMS response does not include Signature key', () => {
      it('Throws Failed to sign JWT with signature error', async () => {
        kmsMock.on(SignCommand)
          .resolves({ KeyId: 'mockKeyId' })

        await (expect(tokenService.mintToken(payload))
          .rejects.toThrow('Failed to sign JWT with signature'))
      })
    })
    ...
    describe('Given KMS provides a valid response', () => {
      it('Returns a token', async () => {
        ...
        expect(result).toEqual('eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6Im1vY2tLaWQifQ.eyJhdWQiOiJodHRwczovL21vY2suYXVkaWVuY2UuY29tIiwiaXNzIjoiaHR0cHM6Ly9tb2NrLmlzc3Vlci5jb20iLCJleHAiOjM2MDAsInNjb3BlIjoibW9jay5zY29wZSIsImNsaWVudF9pZCI6Im1vY2tDbGllbnRJZCJ9.QkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkIkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJA')
      })
    })
  })
})

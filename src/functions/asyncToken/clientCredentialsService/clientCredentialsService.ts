...
export class ClientCredentialsService implements IClientCredentialsService {
  validate = (
    ...
    const hashedSuppliedClientSecret = this.hashSecret(suppliedClientSecret, storedSalt)
    const hashedStoredClientSecret = storedCredentials.hashed_client_secret
    const isValidClientSecret = hashedStoredClientSecret === hashedSuppliedClientSecret

    return isValidClientSecret
  }

  getClientCredentialsById = (storedCredentialsArray: IClientCredentials[], suppliedClientId: string) => {
    const storedCredentials = storedCredentialsArray.find((cred: IClientCredentials) => cred.client_id === suppliedClientId)

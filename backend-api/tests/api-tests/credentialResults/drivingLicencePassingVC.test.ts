...
import { DOCUMENT_NOT_EXPIRED } from "../utils/apiTestData";
...
describe("Driving licence passed credential result", () => {
  ...
  describe("Given the vendor checks pass for a DVLA driving licence", () => {
      ...
      beforeAll(async () => {
        ({ biometricSessionId, sessionId, subjectIdentifier } =
          await doAsyncJourney(Scenario.DRIVING_LICENCE_SUCCESS, {
            drivingLicence: {
              issuedBy: "DVLA",
              validUntil: expiry.getValidUntilTomorrow(),
            },
          }));
      ...
      it("Writes DCMAW_ASYNC_CRI_VC_ISSUED TxMA event with valid properties", () => {
        ...
        expect(actualEvent).toStrictEqual({
          ...
          restricted: {
            ...
            drivingPermit: [
              expect.objectContaining({
                expiryDate: expiry.getValidUntilTomorrow(),
                issuedBy: "DVLA",
              }),
            ],
            address: [expect.any(Object)],
    ...
    describe("Given document expires today", () => {
      beforeAll(async () => {
        ({ biometricSessionId, sessionId, subjectIdentifier } =
          await doAsyncJourney(Scenario.DRIVING_LICENCE_SUCCESS, {
            drivingLicence: {
              issuedBy: "DVLA",
              validUntil: expiry.getValidUntilToday(),
            },
          }));
      ...
      it("Writes DCMAW_ASYNC_CRI_VC_ISSUED TxMA event with valid properties", () => {
        ...
        expect(actualEvent).toStrictEqual({
          ...
          restricted: {
            ...
            drivingPermit: [
              expect.objectContaining({
                expiryDate: expiry.getValidUntilToday(),
                issuedBy: "DVLA",
              }),
            ],
            address: [expect.any(Object)],
    ...
    describe("Given the first day of the expiry grace period is today", () => {
      beforeAll(async () => {
        ({ biometricSessionId, sessionId, subjectIdentifier } =
          await doAsyncJourney(Scenario.DRIVING_LICENCE_SUCCESS, {
            drivingLicence: {
              issuedBy: "DVLA",
              validUntil:
                expiry.getValidUntilForFirstDayOfExpiryGracePeriodToday(),
            },
          }));
    ...
    describe("Given the last day of the expiry grace period is today", () => {
      beforeAll(async () => {
        ({ biometricSessionId, sessionId, subjectIdentifier } =
          await doAsyncJourney(Scenario.DRIVING_LICENCE_SUCCESS, {
            drivingLicence: {
              issuedBy: "DVLA",
              validUntil:
                expiry.getValidUntilForLastDayOfExpiryGracePeriodToday(),
            },
          }));

...
describe("GET /async/activeSession", () => {
  ...
  describe("Given active session is not found", () => {
    it("Returns an error and 404 status code", async () => {
      const accessToken = await getAccessToken("478E3BF8-C3D5-4EA2-A6EE-38B07F2EE0FD")
...
async function getAccessToken(sub: string) {
  ...
  const stsMockResponse = await STS_MOCK_API_INSTANCE.post("/token", requestBody);
...
async function createSessionForSub(sub: string) {
  ...
  const clientIdAndSecretB64 = Buffer.from(clientIdAndSecret).toString("base64")

  // console.log("<<<<< async token basic token >>>>>", clientIdAndSecretB64)

  const asyncTokenResponse = await PROXY_API_INSTANCE.post("/async/token",
  ...
  const credentialTokenResponse = await PROXY_API_INSTANCE.post("/async/credential", {
    sub:
  ...
  {
    headers: { "x-custom-auth": `Bearer ${asyncTokenResponse.data.access_token}` },
  })

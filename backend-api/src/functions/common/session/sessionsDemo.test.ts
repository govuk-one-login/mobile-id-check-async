...
describe("Sessions", () => {
  ...
  it("Demonstrate functionality of new session get/update methods", async () => {
    ...
    const sessionCreatedSessionResult = await adapter.getSessionWithState(
      sessionId,
    ...
    const notFoundSessionResult = await adapter.getSessionWithState(
      sessionId,
    ...
    const invalidUpdateResult = await adapter.updateSession(
      sessionId,
    ...
    const validUpdateResult = await adapter.updateSession(
      sessionId,
      new BiometricTokenIssued(
        "mockDocumentType",
        "mockAccessToken",
        "mockOpaqueId",
      ),
    ...
    const biometricTokenIssuedSessionResult = await adapter.getSessionWithState(
      sessionId,

...
describe("Event Service", () => {
  ...
  describe("Writing active session event to SQS", () => {
    describe(`Given writing "DCMAW_ASYNC_CRI_APP_START" event to SQS fails`, () => {
      beforeEach(async () => {
        sqsMock.on(SendMessageCommand).rejects("Failed to write to SQS");

        result = await eventWriter.writeActiveSessionEvent({
          sub: "mockSub",
          sessionId: "mockSessionId",
          govukSigninJourneyId: "mockGovukSigninJourneyId",
          getNowInMilliseconds: () => 1609462861000,
          ...
          ipAddress: "mockIpAddress",
          txmaAuditEncoded: "mockTxmaAuditEncoded",
          redirect_uri: "https://mockUri.com/redirect",
        });
      });

      it(`Attempts to send "DCMAW_ASYNC_CRI_APP_START" TxMA event to SQS`, () => {
        const expectedCommandInput = {
          MessageBody: JSON.stringify({
            user: {
              user_id: "mockSub",
    ...
    describe(`Given writing "DCMAW_ASYNC_CRI_APP_START" to SQS is successful`, () => {
      describe("Given txmaAuditEncoded is undefined", () => {
        beforeEach(async () => {
          sqsMock.on(SendMessageCommand).resolves({});
        ...
        it(`Attempts to send "DCMAW_ASYNC_CRI_APP_START" event to SQS without restricted data`, () => {
          const expectedCommandInput = {
            MessageBody: JSON.stringify({
              user: {
                user_id: "mockSub",
      ...
      describe("Given txmaAuditEncoded is defined", () => {
        ...
        it(`Attempts to send "DCMAW_ASYNC_CRI_APP_START" event to SQS with restricted data`, () => {
          const expectedCommandInput = {
            MessageBody: JSON.stringify({
              user: {
                user_id: "mockSub",

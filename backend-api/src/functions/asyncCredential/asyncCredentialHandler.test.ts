...
describe("Async Credential", () => {
  ...
  describe("Environment variable validation", () => {
    ...
    describe("Given SESSION_TTL_IN_MS value is not a number", () => {
      it("Returns a 500 Server Error response", async () => {
        dependencies.env = JSON.parse(JSON.stringify(env));
        dependencies.env["SESSION_TTL_IN_MS"] =
        ...
        expect(mockLogger.getLogMessages()[0].data).toStrictEqual({
          errorMessage: "SESSION_TTL_IN_MS is not a valid number",
        });

...
export const convertToAuthSessionCreatedSession: IConvertDynamoDBItemToSession =
  (
    ...
    const authSessionCreatedFields: (keyof AuthSessionCreatedSession)[] =
      commonFields;
...
export const convertToBiometricSessionFinishedSession: IConvertDynamoDBItemToSession =
  (
    ...
    const result = getSessionFieldsAndValues(
      item,
      biometricSessionFinishedFields,
    );
    if (result.isError) {
      return result;
    }
...
function extractValidGovukSigninJourneyId(
  item: DynamoDBItem,
): Result<string, void> {
...
function extractValidSubjectIdentifier(
  item: DynamoDBItem,
): Result<string, void> {
...
function extractValidRedirectUri(
  item: DynamoDBItem,
): SuccessWithValue<string | undefined> {
...
function extractValidBiometricSessionId(
  item: DynamoDBItem,
): Result<string, void> {

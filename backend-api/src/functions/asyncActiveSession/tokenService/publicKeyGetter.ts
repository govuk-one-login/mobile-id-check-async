import { decodeProtectedHeader, importJWK, KeyLike, ProtectedHeaderParameters } from "jose";
import { errorResult, Result, successResult } from "../../utils/result";

export class PublicKeyGetter implements IPublicKeyGetter {
  async getPublicKey(jwt: string, jwks: IJwks): Promise<Result<Uint8Array | KeyLike>> {

    const getDecodedProtectedHeaderResult = await this.getDecodeProtectedHeader(jwt)
  ...
  private async getDecodeProtectedHeader(jwt: string): Promise<Result<ProtectedHeaderParameters>> {
...
export interface IPublicKeyGetter {
  getPublicKey: (jwt: string, jwks: IJwks) => Promise<Result<KeyLike | Uint8Array>>
}
